...
def store_image_to_drive(label, gdrive_id, drive, image_cropped):
...
while True:
  ...
  if str(data_str) == "<image>":
    ...
    if str(data_str) == "</image>":
      ...
      #image_pil.show()
      # Perform center crop to make the image square
      crop_area = (0, 0, h, h)
      image_cropped = image_pil.crop(crop_area)
      image_cropped.show()
      
      store_image_to_drive(label, gdrive_id, drive, image_cropped)
    else:
